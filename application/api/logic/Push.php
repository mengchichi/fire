...
class Push
{
    private $app_key = '59f1b24f26cd43f6ee0cba29';        //待发送的应用程序(appKey)，只能填一个。
    private $master_secret ='04b0f8e7416632baacd4585a';    //主密码
    ...
    */
    ...
    //调用推送方法
    public static function send_pub($userID,$content = '',$m_txt)
    {
        $push = new Push();
        //$m_type = '';//推送附加字段的类型
        $m_txt = json_encode($m_txt);//推送附加字段的类型对应的内容(可不填) 可能是url,可能是一段文字。
        $m_time = '86400';//离线保留时间
        $receive = [
            'alias' => [$userID]
        ];
